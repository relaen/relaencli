...
export class SqlServerGenerator extends BaseGenerator {
    ...
    async getConn() {
        ...
        this.config.schema = this.config.options.schema || 'dbo';
    ...
    async genTables(conn: any) {
        ...
        let results = await conn.request().input('0', this.config.schema).query(sql);
    ...
    async getFields(conn: any, tableName: string): Promise<IColumn[]> {
        let results = await conn.request().input('0', tableName).input('1', this.config.schema).input('2',this.config.schema + '.' + tableName)
        ...
        let identitys = await conn.request().input('0', this.config.schema + '.' + tableName).query(`select * from syscolumns where id=object_id(@0) and status=0x80`);
        let arr: IColumn[] = [];
    ...
    async genRelations(conn: any) {
        ...
        let result = await conn.request().input('0', this.config.schema).query(sql);

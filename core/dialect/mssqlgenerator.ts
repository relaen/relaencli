...
class MssqlGenerator extends BaseGenerator {
    ...
    async getFields(conn: any, tableName: string): Promise<IColumn[]> {
        ...
        let uniq = await conn.request().input("0", tableName)
            .query(`SELECT t2.COLUMN_NAME FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS  AS t1 
                JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS t2 ON t1.CONSTRAINT_NAME  = t2.CONSTRAINT_NAME  
                AND t1.CONSTRAINT_TYPE= 'UNIQUE' AND t1.TABLE_NAME=@0`);
        let uniqArr: string[] = [];
        if (uniq) {
            for (let cn of uniq.recordset) {
                uniqArr.push(cn['COLUMN_NAME'])
            }
        }
        let arr: IColumn[] = [];
        for (let f of results.recordset) {
            let column = {
                ...
                isUnique: uniqArr.indexOf(f.name) === -1 ? false : true
            }

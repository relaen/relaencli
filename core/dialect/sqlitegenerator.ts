...
class SqliteGenerator extends BaseGenerator {
    ...
    async getFields(conn: any, tableName: string): Promise<IColumn[]> {
        ...
        let bulidSql: string = await new Promise((resolve, reject) => {
            conn.all("SELECT * from  sqlite_master where tbl_name = '" + tableName + "' and type ='table'", (err, rows) => {
                if (err) return reject(err);
                resolve(rows);
            });
        });
        let isUni = bulidSql.indexOf("UNIQUE");
        let isUniArr = []
        if (isUni != -1) {
            bulidSql = bulidSql.slice(isUni, -1);
            var str = bulidSql.split("UNIQUE");
            for (let sl of str) {
                let cName = sl.split('"')[1];
                isUniArr.push(cName);
            }
        ...
        for (let r of results) {
            arr.push({
                ...
                isUnique: isUniArr.indexOf(r.name) === -1 ? false : true
            })
        }
        return arr;
    }

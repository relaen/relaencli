...
class BaseGenerator {
    constructor(config) {
        ...
        this.tables = new Map();
        //读取数据库类型对象
        this.typeMap = dbtype_1.Dbtype[this.config.dialect];
    }
    /**
     * 创建所有实体
     */
     ...
     */
    async genEntity(conn, entityName) {
        let tn = this.tables.get(entityName);
        //引入relaen对象
        let relaenArr = ['BaseEntity', 'Entity', 'Column'];
        ...
        let primaryType;
        //字段集合 
        let fieldArr = await this.getFields(conn, tn);
        for (let r of fieldArr) {
            ...
            let relObj = this.getRelation(entityName, r.field);
            //默认字符串
            let type;
            //字段名
            let fn;
            ...
            if (!relObj || r.isPri) {
                ...
                entityArr.push(arr.join("," + util_1.Util.getLineChar()));
                entityArr.push("\t})");
            ...
            if (relObj) { //引用字段
                ...
                let relName = r.isPri ? 'OneToOne' : 'ManyToOne';
                if (r.isPri) {
        ...
        //one to many
        if (primaryKey) {
            let arr = this.getRefered(entityName, primaryKey);
            if (arr.length > 0) {
        ...
        for (let a of getterFieldArr) {
            ...
            let type = a['type'];
            //首字母大写
            let bigP = fn.substr(0, 1).toUpperCase() + fn.substr(1);
            //getter方法
            if (a['ref']) {
        ...
        return entityArr.join(util_1.Util.getLineChar());
    }
    /**
     * 处理relation为标准relation
     * @param relArr    关联数组
     */
    handleRelation(relArr) {
        let map = new Map();
        //1 按ref entity归类
        for (let o of relArr) {
        ...
        for (let o of map) {
            if (o[1].length > 1) {
                for (let o1 of o[1]) {
                    let cn = this.genName(o1.column, this.config.columnSplit, this.config.columnStart, 0);
                    //many to one 引用名
